 ...
 */
function unflattenObject(flattened: { [key: string]: any }): any {
  const result: any = {};
 ...
 */
async function sheetsToMixpanel(spreadsheetUrl: string) {
  try {
    ...
    const sheetTitles = spreadsheet.data.sheets?.map(sheet => sheet.properties?.title).filter(Boolean) as string[];
    ...
    for (const sheetTitle of sheetTitles) {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        await new Promise<void>((resolve, reject) => {
          mixpanel.track(eventType, eventProperties, (err: any) => {
            if (err) {
